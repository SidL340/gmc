...
import { Smartphone, ArrowRight, KeyRound, User, Loader2 } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (code) {
        ...
        toast.success(`6-digit code sent to ${phone}`);
      }
      setStep('otp');
    } catch (err: any) {
  ...
  const handleCompleteRegistration = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const res = await api.post('/api/auth/register', { phone, code: otp, name: name.trim() });
      const data = res.data.data;
      setAuth(data.user, data.accessToken, data.refreshToken);
    ...
    <div className="min-h-[75vh] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md bg-white rounded-3xl p-8 border border-gray-100 shadow-xl space-y-6">
        ...
        <div className="text-center space-y-1">
          <div className="w-12 h-12 bg-primary-100 text-primary-600 rounded-2xl flex items-center justify-center mx-auto mb-3 font-bold text-xl">
            GM
          </div>
          <h2 className="text-2xl font-serif font-bold text-gray-900">
            {step === 'phone' && 'Login or Register'}
            {step === 'otp' && 'Enter Verification Code'}
            {step === 'register' && 'Tell Us Your Name'}
          </h2>
          <p className="text-xs text-gray-500">
            {step === 'phone' && 'Enter your mobile number to get an instant OTP'}
            {step === 'otp' && `Sent to +977 ${phone}`}
            {step === 'register' && 'Almost there! Set up your customer profile'}
          </p>
        </div>
          ...
          <form onSubmit={handleSendOtp} className="space-y-4">
            <div>
              ...
              <div className="flex gap-2">
                <span className="inline-flex items-center px-3.5 py-2 rounded-xl bg-gray-100 text-gray-700 text-xs font-semibold border border-gray-200">
                  🇳🇵 +977
                </span>
                <input
                  type="tel"
        ...
        {step === 'otp' && (
          <form onSubmit={handleVerifyOtp} className="space-y-4">
            ...
            <button
              ...
              className="w-full py-3.5 bg-primary-600 hover:bg-primary-700 text-white font-bold text-sm rounded-xl shadow-md transition-all flex items-center justify-center gap-2 disabled:opacity-40"
            >
              {isLoading ? <Loader2 size={16} className="animate-spin" /> : <>Verify & Login <KeyRound size={16} /></>}
            </button>
